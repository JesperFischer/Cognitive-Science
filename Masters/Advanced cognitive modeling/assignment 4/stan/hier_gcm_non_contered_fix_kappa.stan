...
transformed parameters {
    ...
    // parameter c 
    vector<lower=0,upper=2>[nsubjects] c_Z = inv_logit(logit_c_Z)*2;  // times 2 as c is bounded between 0 and 2
    real<lower=0,upper=2> c_mu = inv_logit(logit_c_M)*2;  // times 2 as c is bounded between 0 and 2
    ...
    for (sub in 1:nsubjects) {
      for (trial in 1:ntrials) {
        ...
        for (e in 1:(trial-1)){
            ...
            exemplar_sim[e] = exp(-(c_mu+c_Z[sub]) * sum(tmp_dist));
        }
...
model {
    ...
    target += std_normal_lpdf(to_vector(logit_c_Z));
