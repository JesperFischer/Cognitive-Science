data {
  ...
  int rw1[trials,subjects];
  
  int rw2[trials,subjects];
  int fb_rw1[trials,subjects];
  int fb_rw2[trials,subjects];
...
parameters {
  ...
  real bias_2_mu;
  real bias_2_sd;
  vector[subjects] bias_1;
  vector[subjects] bias_2;
}
...
generated quantities{
  ...
  int sim_rw1[subjects];
  int sim_rw2[subjects];
  
  real <lower = 0, upper = 1> theta1_prior_p = inv_logit(bias_1_mu);
  real <lower = 0, upper = 1> theta2_prior_p = inv_logit(bias_2_mu); 
  
  real <lower = 0, upper = 1> theta1_prior[subjects];
  real <lower = 0, upper = 1> theta2_prior[subjects]; 
